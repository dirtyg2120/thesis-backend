...
def login(auth_details: schemas.AuthDetails):
    ...
    if (user is None) or (not operator_auth_handler.verify_password(auth_details.password, user['password'])):
...
@router.get('/op_protected', response_model=schemas.UserIdentifier, name="operator:test")
