...
class SOBOG(torch.nn.Module):
    def __init__(self, gpu: int, **kwargs):
        ...
        self.post_classifier = nn.ModuleList(
            [
                nn.Linear(kwargs["d_post_embed"], kwargs["d_cls"])
                if i == 0
                else nn.Linear(kwargs["d_cls"], 1)
                if i == kwargs["n_cls_layer"] - 1
                else nn.Linear(kwargs["d_cls"], kwargs["d_cls"])
                for i in range(kwargs["n_cls_layer"])
            ]
        )
        ...
        self.user_classifier = nn.ModuleList(
            [
                nn.Linear(
                    kwargs["d_user_embed"] + kwargs["d_post_embed"], 2 * kwargs["d_cls"]
                )
                if i == 0
                else nn.Linear(2 * kwargs["d_cls"], 1)
                if i == kwargs["n_cls_layer"] - 1
                else nn.Linear(2 * kwargs["d_cls"], 2 * kwargs["d_cls"])
                for i in range(kwargs["n_cls_layer"])
            ]
        )
